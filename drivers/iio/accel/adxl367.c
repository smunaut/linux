...
#define ADXL367_THRESH_H_TO_VAL(x)	(((x) & ADXL367_THRESH_H_MASK) << 6)
#define ADXL367_THRESH_L_TO_VAL(x)	(((x) & ADXL367_THRESH_L_MASK) >> 2)
#define ADXL367_THRESH_TO_VAL(h, l)	(ADXL367_THRESH_H_TO_VAL(h) \
					 | ADXL367_THRESH_L_TO_VAL(l))
...
static int adxl367_update_fifo_samples(struct adxl367_state *st)
{
	unsigned int fifo_samples = st->fifo_watermark * st->fifo_set_size;
	int ret;
...
static int adxl367_set_watermark(struct iio_dev *indio_dev, unsigned int val)
{
	...
	ret = adxl367_update_fifo_samples(st);
	if (ret)
		return ret;
					 ...
					 enum adxl367_fifo_format *fifo_format)
{
	...
	for (i = 0; i < ARRAY_SIZE(adxl367_channel_masks); i++)
		if (*scan_mask == adxl367_channel_masks[i])
			break;

	if (i == ARRAY_SIZE(adxl367_channel_masks))
		return false;
